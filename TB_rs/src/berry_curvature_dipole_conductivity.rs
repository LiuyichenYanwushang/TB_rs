...
///Berry curvature dipole parameter
#[derive(Debug, Copy, Clone, Serialize, Deserialize)]
pub struct BCD_parameter {
...
pub fn berry_curvature_dipole_onek<S: Data<Elem = f64>>(
    model: &Model,
    k_vec: &ArrayBase<S, Ix1>,
    T : f64,
    mu: &Array1<f64>,
) -> Array2<f64> {
    //!给定一个 k 点, T 代表温度, og= $\og$,
    //!mu=$\mu$ 为费米能级范围,
    //! 这个函数返回的是某个k点在某个温度下的berry curvature_dipole.
    //! 公式为 $-\frac{\p f_n}{\p \epsilon} v_\gamma\Omega_n^{\alpha\beta}$
    ...
    let UU0 = UU.mapv(|x| if x.abs() < 1e-8 { 0.0 } else { 1.0 / x.powi(2) });//平方相减

    let v_x=A_x.diag().to_owned().mapv(|x| x.re);
    let v_y=A_y.diag().to_owned().mapv(|x| x.re);
    let v_z=A_z.diag().to_owned().mapv(|x| x.re);

    let n_mu = mu.len();
    let mut berry_curvature_dipole = Array2::zeros((9, n_mu));
    let mut fermi=Array2::zeros((band.len(),n_mu));
    let beta = 1.0 / T / 8.617e-5;
    Zip::from(fermi.columns()).and(mu.view()).for_each(|pf,u| {band.mapv(|x| ((beta * (x - u)).exp() + 1.0).recip());});
    let par_fermi:Array2<f64>=&fermi*(1.0-&fermi)*beta;

    let A_xy = &A_x * (&A_y.t());
    let A_yz = &A_y * (&A_z.t());
    let A_xz = &A_x * (&A_z.t());
    let A_xy = A_xy.mapv(|x| -2.0*x.im);
    let A_yz = A_yz.mapv(|x| -2.0*x.im);
    let A_xz = A_xz.mapv(|x| -2.0*x.im);
    ...
    par_fermi.axis_iter(Axis(1)).zip(berry_curvature_dipole.axis_iter_mut(Axis(1))).for_each(|(pf, mut O)| {
            O[[0]] =pf.dot(&A_xyx);
            O[[1]] =pf.dot(&A_xyy); 
            O[[2]] =pf.dot(&A_xyz); 
            O[[3]] =pf.dot(&A_yzx);
...
pub fn berry_curvature_dipole_conductivity(
    model: &Model,
    kvec: &Array2<f64>,
    bcd_parameter: BCD_parameter,
) -> Array2<f64> {
    let li: Complex<f64> = 1.0 * Complex::i();
    let mu = bcd_parameter.mu();
    let n_mu = mu.len();
    let T=bcd_parameter.temperature();
    let mut sigma = Array2::zeros((9, n_mu));
    //let mut a = Instant::now();
    for (i, k) in kvec.outer_iter().enumerate() {
        let berry_curvature_dipole = berry_curvature_dipole_onek(&model, &k, T, &mu);
        sigma.add_assign(&berry_curvature_dipole);
    }
    sigma
}


pub fn berry_curvature_dipole_conductivity_calculate(
    ...
    if rank == 0 {
        ...
        let mut bcd_parameter = BCD_parameter::new();
        let have_kmesh = bcd_parameter.get_k_mesh(&Input_reads);
        if !have_kmesh {
            writeln!(
                output_file.as_mut().unwrap(),
        ...
        let have_T = bcd_parameter.get_T(&Input_reads);
        let have_mu = bcd_parameter.get_mu(&Input_reads);

        if !(have_T) {
            writeln!(output_file.as_mut().unwrap(),"Warning: You don't specify temperature when calculate the anomalous hall conductivity, using default 1 K");
        }
        if !(have_mu) {
        ...
        let kvec = bcd_parameter.get_mesh_vec();

        //传输 bcd_parameter
        let mut serialized_data = serialize(&bcd_parameter).unwrap();
        ...
        let mut conductivity: Array2<f64> =
            berry_curvature_dipole_conductivity(&model, &chunk, bcd_parameter);
        ...
        conductivity = conductivity / (nk as f64) / model.lat.det().unwrap()
            ...
            * 1.0e8;
        println!("The berry curvature dipole conductivity calculation is finished");
        writeln!(output_file.as_mut().unwrap(), "calculation finished");
        //-------------------------------开始写入-----------------------
        writeln!(
            output_file.as_mut().unwrap(),
            "write data in berry_curvature_dipole_conductivity.dat"
        );
        let mut BCD_file = File::create("berry_curvature_dipole_conductivity.dat")
            .expect("Unable to create spin_current_conductivity.dat");
        let mut input_string = String::new();
        input_string.push_str("#Calculation results are reported in units of Ω^-1 V^-1\n");
        input_string.push_str("#The arranged data are: mu,  xx,  yy,  zz,  xy,  yz,  xz,  yx,  zy  zx. The order of the subscripts is Ω_a v_b\n");
        ...
        writeln!(BCD_file, "{}", &input_string);

        //---------------------开始绘图------------------------

        for (row_idx, component) in ["xx", "yy", "zz", "xy", "yz", "xz", "yx", "yz", "zx"].iter().enumerate() {
            ...
            let mut pdf_name = format!("BCD_{}.pdf", component);
        ...
        // 反序列化
        let bcd_parameter: BCD_parameter = deserialize(&received_data).unwrap();
        //接受 bcd_paramter 结束, 开始接受kvec
        let mut nk: usize = 0;
        world.process_at_rank(0).broadcast_into(&mut nk);
        ...
        let conductivity: Array2<f64> = berry_curvature_dipole_conductivity(&model, &chunk, bcd_parameter);
